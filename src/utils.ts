...
export const validateEnvironment = (
  ...
  if (!skipValidations) {
    try {
      ...
      logger.debug(error);
      throw new Error(
        `Fatal error while running validations. ${error.message}`,
      );
    }
  }
};
...
export const handleError = (error: any): void => {
  ...
  logger.error(
    chalk.red(error.message.replaceAll(WOLLOK_EXTRA_STACK_TRACE_HEADER, "")),
  );
...
export const valueDescription = (val: any): string =>
  chalk.italic(chalk.blue(val));
...
export const publicPath = (...paths: string[]): string =>
  path.join(import.meta.dirname || "", "..", "public", ...paths);
...
export const isValidAsset = (file: Named): boolean =>
  assetsExtensions.some((extension) => file.name.endsWith(extension));
export const isValidImage = (file: Named): boolean =>
  VALID_IMAGE_EXTENSIONS.some((extension) => file.name.endsWith(extension));
export const isValidSound = (file: Named): boolean =>
  VALID_SOUND_EXTENSIONS.some((extension) => file.name.endsWith(extension));

// ══════════════════════════════════════════════════════════════════════════════════════════════════════════════════
// WOLLOK AST
// ══════════════════════════════════════════════════════════════════════════════════════════════════════════════════

export function isREPLConstant(
  environment: Environment,
  localName: string,
): boolean {
