...
program.command("run")
  ...
  .option(
    "-a, --assets [path]",
    "path relative to project for game assets. By default, it takes the assets definition from package.json.",
    "assets",
  )
  ...
  .option(
    "-d, --startDiagram",
    "activate the dynamic diagram (only for games)",
    false,
  )
  .action((programFQN, options) => {
    run(programFQN, options);
  });
...
program.command("repl")
  ...
  .option(
    "--skipDiagram",
    "avoid starting the server for the dynamic diagram",
    false,
  )
  .option("--host [host]", "host to run (bind) the server", "localhost")
  .option("--port [port]", "port to run the server", "3000")
...
program.command("init")
  ...
  .argument(
    "[folder]",
    "folder name, if not provided, the current folder will be used",
  )
  .option("-p, --project [filter]", "path to project", process.cwd())
