...
export default async function (
  ...
  try {
    logger.info(
      `${game ? gameIcon : programIcon} Running program ${
        valueDescription(programFQN)
      } ${runner(game)} on ${valueDescription(project)}`,
    );
    options.assets = game ? getAssetsFolder(options) : "";
    if (game) {
      const logGameFinished = (exitCode: any) => {
        fileLogger.info({
          message: `${gameIcon} Game executed ${programFQN} on ${project}`,
      ...
      logger.info(
        `${folderIcon}  Assets folder ${join(project, options.assets)}`,
      );
      Array.from([
        "exit",
        "SIGINT",
    ...
    logger.info(
      `${buildEnvironmentIcon} Building environment for ${
        valueDescription(programFQN)
      }...${ENTER}`,
    );
    ...
    const interpreter = game
      ? getGameInterpreter(environment)
      : interpret(environment, { ...natives });
    const programPackage = environment.getNodeByFQN<Package>(programFQN)
      .parent as Package;
    const dynamicDiagramClient = await initializeDynamicDiagram(
      programPackage,
      options,
      interpreter,
    );
    ...
    if (!game) {
      fileLogger.info({
        message: `${programIcon} Program executed ${programFQN} on ${project}`,
        timeElapsed: timeMeasurer.elapsedTime(),
        ok: true,
      });
      process.exit(0);
    ...
    handleError(error);
    fileLogger.info({
      message: `${game ? gameIcon : programIcon} ${
        game ? "Game" : "Program"
      } executed ${programFQN} on ${project}`,
      timeElapsed: timeMeasurer.elapsedTime(),
      ok: false,
...
export const initializeGameClient = (
  { project, assets, host, port, game }: Options,
): Server | undefined => {
  ...
  if (soundsFolder !== assets) {
    app.use(
      cors({ allowedHeaders: "*" }),
      express.static(soundsFolder, { maxAge: "1d" }),
    );
  }
  ...
  logger.info(
    successDescription(
      "Game available at: " +
        chalk.bold(`http://${currentHost}:${currentPort}`),
    ),
  );
...
export function initializeDynamicDiagram(
  ...
  const connectionListener = (interpreter: Interpreter) => (socket: Socket) => {
    ...
    socket.emit(
      "updateDiagram",
      getDynamicDiagram(interpreter, programPackage),
    );
  };
...
export const eventsFor = (
  io: Server,
  interpreter: Interpreter,
  dynamicDiagramClient: DynamicDiagramClient,
  assetFiles: Asset[],
): void => {
  io.on("connection", (socket) => {
    ...
    socket.on("keyPressed", (events: string[]) => {
      queueEvent(
        interpreter as any,
        ...events.map((code) => buildKeyPressEvent(interpreter as any, code)),
      );
    });
    ...
    const id = setInterval(() => {
      try {
        ...
        interpreter.send(
          "flushEvents",
          gameSingleton,
          interpreter.reify(start.elapsedTime()),
        );
        draw(interpreter, io);
        profiler.stop();
...
export const getAllAssets = (
  ...
  const fileRelativeFor = (fileName: string) => ({
    name: fileName,
    url: fileName,
  });
  ...
  const loadAssetsIn = (basePath: string): Asset[] =>
    fs.readdirSync(basePath, { withFileTypes: true })
      .flatMap((file: Dirent) =>
        file.isDirectory()
          ? loadAssetsIn(join(basePath, file.name))
          : isValidAsset(file)
          ? [fileRelativeFor(relative(baseFolder, join(basePath, file.name)))]
          : []
      );
...
export const getVisuals = (
  ...
  (game.get("visuals")?.innerCollection ?? []).map((visual) =>
    visualState(interpreter as any, visual as any)
  );

export const getSounds = (game: RuntimeObject): SoundState[] =>
  (game.get("sounds")?.innerCollection ?? [] as any).map(soundState);

export const getSoundsFolder = (
  projectPath: string,
  assetsOptions: string | undefined,
): string =>
  fs.readdirSync(projectPath).includes("sounds")
    ? "sounds"
...
export const buildEnvironmentForProgram = async (
...
export const runner = (game: boolean): string =>
  game ? "as a game" : "as a program";

export const gamePort = (port: string): string => port ?? DEFAULT_PORT;
export const gameHost = (host: string): string => host ?? DEFAULT_HOST;

export const dynamicDiagramPort = (port: string): string =>
  `${+gamePort(port) + 1}`;
    ...
    logger.error(
      failureDescription(
        error instanceof WollokException
          ? error.message
          : "Exception while executing the program",
      ),
    );
    const debug = logger.getLevel() <= logger.levels.DEBUG;
