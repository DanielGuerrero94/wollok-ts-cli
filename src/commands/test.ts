...
export function validateParameters(
  ...
  { file, describe, test }: Options,
): void {
  if (filter && (file || describe || test)) {
    throw new Error(
      "You should either use filter by full name or file/describe/test.",
    );
  }
}

export function matchingTestDescription(
  filter: string | undefined,
  options: Options,
): string {
  if (filter) return `matching ${valueDescription(filter)}`;
  if (options.file || options.describe || options.test) {
    const stringifiedOrWildcard = (value?: string) =>
      value ? `'${value}'` : "*";
    return `matching ${
      valueDescription(
        [options.file, options.describe, options.test].map(
          stringifiedOrWildcard,
        ).join("."),
      )
    }`;
  }
...
export function getTarget(
  ...
  try {
    possibleTargets = getBaseNode(environment, filter, options).descendants
      .filter(getTestFilter(filter, options));
    const onlyTarget = possibleTargets.find((test: Test) => test.isOnly);
    const testMatches = (filter: string) => (test: Test) =>
      !filter || sanitize(test.fullyQualifiedName)!.includes(filter);
    const filterTest = sanitize(filter) ?? "";
    return onlyTarget
      ? [onlyTarget]
      : possibleTargets.filter(testMatches(filterTest));
  } catch (e: any) {
...
function getBaseNode(
  ...
  if (file) {
    nodeToFilter = nodeToFilter.descendants.find((node) =>
      node.is(Package) && node.fileName === file
    ) as Package | undefined;
    if (!nodeToFilter) {
      throw new TestSearchMissError(`File '${file}' not found`);
    }
  }
  if (describe) {
    nodeToFilter = nodeToFilter.descendants.find((node) =>
      node.is(Describe) && node.name === `"${describe}"`
    ) as Describe | undefined;
    if (!nodeToFilter) {
      throw new TestSearchMissError(`Describe '${describe}' not found`);
    }
  }
  return nodeToFilter;
}
...
function getTestFilter(
  filter: string | undefined,
  options: Options,
): (node: Node) => node is Test {
  return filter || !options.test
    ? is(Test)
    : (node: Node): node is Test =>
      node.is(Test) && node.name === `"${options.test}"`;
}
export function tabulationForNode(
  { fullyQualifiedName }: { fullyQualifiedName: string },
): string {
...
export default async function (
  ...
  try {
    ...
    const runAllTestsDescription = `${testIcon} Running all tests${
      matchLog ? ` ${matchLog} ` : " "
    ...
    failures.forEach(([test, error]) => {
      log();
      logger.error(
        failureDescription(chalk.bold(test.fullyQualifiedName), error),
      );
    });
    ...
    fileLogger.info({
      message: `${testIcon} Test runner executed ${
        filter ? `matching ${filter} ` : ""
      }on ${project}`,
      result: { ok: successes, failed: failures.length },
      failures: failuresForLogging,
